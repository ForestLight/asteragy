...
namespace Asteragy.Game {
    ...
    public class Cursor : IInputable {
        ...
        public Cursor(ContentManager content, AsterPosition positions) {
            ...
            move = new InterpolateOnce<Vector2>(Vector2.SmoothStep, data.MoveTime);
        }
        ...
        public void Move(Direct direct) {
            ...
            while(check != Position) {
                if(Movable == null || Movable[check.X, check.Y]) {
                    move.Restart(positions[Position.X, Position.Y] - positions[check.X, check.Y], Vector2.Zero);
                    Position = check;
                    break;
                }
                method(ref check);
        ...
        public void Draw(GraphicsDevice device, SpriteBatch sprite) {
            ...
            sprite.Draw(data.CursorTexture, positions[Position.X, Position.Y] + move.Now, null, Color.White, 0.0f, data.Center, 1.0f, SpriteEffects.None, 0.0f);
            sprite.End();
        }
